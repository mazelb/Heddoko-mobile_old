...
extern "C" void CreateUnityRenderBuffersMTL(UnityDisplaySurfaceMTL* surface)
{
	...
	if(surface->targetAAColorRT)
		...
		surface->unityColorBuffer = UnityCreateDummySurfaceMTL(surface->unityColorBuffer, true, &sys_desc);
	...
	{
		surface->systemColorBuffer = UnityCreateDummySurfaceMTL(surface->systemColorBuffer, true, &sys_desc);
		surface->systemDepthBuffer = UnityCreateDummySurfaceMTL(surface->systemDepthBuffer, false, &sys_desc);
	}
	else
	{
...
extern "C" void DestroyUnityRenderBuffersMTL(UnityDisplaySurfaceMTL* surface)
{
	...
	if(surface->unityColorBuffer)	UnityDestroyExternalSurfaceMTL(surface->unityColorBuffer);
	if(surface->systemColorBuffer)	UnityDestroyExternalSurfaceMTL(surface->systemColorBuffer);
	surface->unityColorBuffer = surface->systemColorBuffer = 0;

	if(surface->unityDepthBuffer)	UnityDestroyExternalSurfaceMTL(surface->unityDepthBuffer);
	if(surface->systemDepthBuffer)	UnityDestroyExternalSurfaceMTL(surface->systemDepthBuffer);
